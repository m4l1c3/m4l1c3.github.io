...
I was able to upload the agent binaries we got earlier to ropshell.com which analyzes the compiled code for items in memory that can be used in an exploit, in this case: call eax.  Ater reading through some reference docs for assembly, specifically x86 I have surmised that with our goal being getting a root shell by escaping the 'agent' service causing a buffer overflow to occur and running 'call eax.'  In doing so we would be calling eax which is potentially a reference to the parent process of the service, in this case a shell runnint as root.  I'm basing this on the concept that eax register in the processor can be used to execute code in the register to circumvent normal program execution.
